...
public class EnhancementSystem : MonoBehaviour
{
    ...
    /// <summary>EquipmentSlot(신규 인벤 슬롯)에서 직접 호출하는 강화 선택</summary>
    public void SelectItemForEnhancementDirect(EquipmentData equipment, int enhanceLevel, int itemLevel, int count)
    {
        ...
        Debug.Log($"[EnhancementSystem] 장비 슬롯 직접 선택: {equipment.itemName} +{currentEnhanceLevel}");
    }
    ...
    private void OnEnhanceSuccess()
    {
        ...
        else
        {
            ...
            // equipUnlockMap 동기화 (인벤토리 탭 전환 시에도 강화 수치 유지)
            if (currentEquipment != null && InventoryManager.Instance != null)
            {
                int count = InventoryManager.Instance.GetItemCount(currentEquipment);
                InventoryManager.Instance.SyncEquipSlotToMap(
                    currentEquipment.itemID, count, currentEnhanceLevel,
                    currentInventorySlot != null ? currentInventorySlot.itemLevel : 0);
            }
        ...
        Debug.Log($"[EnhancementSystem] 강화 성공! {currentEquipment?.itemName} +{currentEnhanceLevel}");
