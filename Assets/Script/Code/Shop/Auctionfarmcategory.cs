...
/// 기존 AuctionManager와 독립적으로 Farm 전용 경매 목록을 관리.
///
/// [Inspector 연결]
///   categoryDropdown     : 경매장 카테고리 TMP_Dropdown
///   farmRegisterPanel    : 농산물 등록 팝업 패널
///   farmAuctionContent   : 농산물 경매 목록 스크롤뷰 컨텐츠
...
public class AuctionFarmCategory : MonoBehaviour
{
    ...
    private void SetupDropdown()
    {
        ...
        Debug.Log("[AuctionFarmCategory] 드롭다운에 농산물 카테고리 추가 완료");
    }
    ...
    private void OnConfirmRegister()
    {
        ...
        int quantity = pendingRegisterItem.count; // 전량 등록 (원하면 InputField 추가)
        ...
        FarmInventoryConnector.Instance?.RegisterFarmCropToAuction(
            pendingRegisterItem.itemName, quantity, startBid, buyout, duration);
    ...
    private void OnFarmAuctionEnded(FarmAuctionListing listing)
    {
        ...
        else
        {
            ...
            FarmInventoryConnector.Instance?.AddFarmCrop(
                ...
                listing.itemType, listing.startingBid);
