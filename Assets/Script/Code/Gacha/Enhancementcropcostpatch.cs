...
///
/// ※ FarmManager.SpendCropPoints() 와 GameManager.SpendGold() 는
///   이미 프로젝트에 존재하므로 이 클래스에서 직접 호출합니다.
/// </summary>
public class EnhancementCropCostPatch : MonoBehaviour
{
    ...
    [Tooltip("강화 레벨별 CP 증가 배율 (레벨^배율 적용)")]
    ...
    public bool CanEnhance(int currentEnhanceLevel, int goldCost)
    {
        ...
        if (GameManager.Instance == null || GameManager.Instance.PlayerGold < goldCost)
        {
            UIManager.Instance?.ShowMessage(
                $"골드 부족! {goldCost:N0}G 필요 (보유 {GameManager.Instance?.PlayerGold ?? 0:N0}G)", Color.red);
            return false;
        }
        ...
        {
            UIManager.Instance?.ShowMessage(
                $"작물 포인트 부족! {cpCost}CP 필요 (보유 {curCp}CP)", Color.red);
            return false;
        }
    ...
    // ─────────────────────────────────────────────────────────
    //  비용 지불 (Gold + CropPoint 차감)
    // ─────────────────────────────────────────────────────────

    /// <summary>
    /// Gold + CropPoint 차감.
    /// 성공 시 true, 부족 시 false (메시지 표시 포함).
    /// </summary>
