...
public class EquipmentManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // ★ Fix: 씬 전환 후 PlayerStats가 재생성될 때 장비 스탯 재적용
        // (씬 로드 완료 시점이므로 PlayerStats.Instance가 반드시 존재)
        RecalculateStats();
    }
    ...
    private void ApplyEquipmentStats(EquipmentData eq, int enhanceLevel, bool apply)
    {
        ...
        float bonus = 1f + (enhanceLevel * 0.1f); // 강화 레벨 보너스 배율

        // ✅ 핵심 수정: equipmentStats 구조체에서 읽도록 변경
        // 기존에는 eq.attackBonus를 읽었지만, 
        // 인스펙터에서 수정하는 실제 값은 eq.equipmentStats.attack에 들어있음
        PlayerStats.Instance.bonusAttack += eq.equipmentStats.attack * bonus * sign;
        PlayerStats.Instance.bonusDefense += eq.equipmentStats.defense * bonus * sign;
        PlayerStats.Instance.bonusMaxHp += eq.equipmentStats.health * bonus * sign;
        PlayerStats.Instance.bonusSpeed += eq.equipmentStats.moveSpeed * bonus * sign;
        PlayerStats.Instance.bonusCritical += eq.equipmentStats.criticalRate * bonus * sign;
    ...
    public void LoadEquipmentSaveData(EquipmentSaveData data)
    {
        ...
        foreach (var slotData in data.slots)
        {
            ...
            // ItemDatabase에서 장비 검색
            EquipmentData eq = ItemDatabase.Instance?.GetEquipmentByID(slotData.itemID);
            if (eq == null)
            {
                Debug.LogWarning($"[EquipmentManager] Load: itemID={slotData.itemID} 장비 없음");
                continue;
            }
