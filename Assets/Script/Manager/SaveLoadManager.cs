...
public class SaveLoadManager : MonoBehaviour
{
    ...
    private void ApplySaveData(SaveData data)
    {
        ...
        if (EquipmentManager.Instance != null && data.equipmentData != null)
        {
            ...
            if (ItemDatabase.Instance != null && ItemDatabase.Instance.IsReady)
            {
                ...
                // ✅ [SPUM 비주얼 수정] 즉시 로드 경로에서도 SPUM 갱신
                FindObjectOfType<SPUMEquipmentVisualSystem>()?.UpdateAllEquipmentVisuals();

                // ✅ [체력바 수정] 장비 bonusMaxHp 적용 후 HP/마나 최대치 복원
                // (저장 당시 낮은 HP비율로 RecalculateMaxHp하면 HP바가 낮게 표시됨)
                var ps2 = FindObjectOfType<PlayerStats>();
                if (ps2 != null)
                {
                    ps2.currentHealth = ps2.maxHealth;
                    ps2.currentMana = ps2.maxMana;
                    ps2.UpdateStatsUI();
                    Debug.Log($"[SaveLoadManager] HP/마나 최대치 복원: {ps2.maxHealth}/{ps2.maxMana}");
                }
            }
            else
            {
    ...
    private System.Collections.IEnumerator WaitAndLoadEquipment(EquipmentSaveData equipmentData)
    {
        ...
        // ✅ [Bug5 수정] 장비 지연 로드 후 스킬/비주얼/체력 재구성
        yield return null;  // 1프레임 대기 (PlayerStats.RecalculateStats 반영)
        ...
        // ✅ [SPUM 비주얼 수정] 장비 로드 후 캐릭터 외형 갱신
        // SPUMEquipmentVisualSystem.Start()는 장비 로드보다 먼저 실행되어 빈 상태로 초기화
        // → 장비 로드 완료 후 명시적으로 UpdateAllEquipmentVisuals 호출
        var spumVisual = UnityEngine.Object.FindObjectOfType<SPUMEquipmentVisualSystem>();
        if (spumVisual != null)
        {
            spumVisual.UpdateAllEquipmentVisuals();
            Debug.Log("[SaveLoadManager] 장비 지연 로드 후 SPUM 비주얼 갱신 완료");
        }

        // ✅ [체력바 수정] 장비의 bonusMaxHp가 적용된 후 HP/마나를 만빵으로 복원
        // RecalculateMaxHp()가 저장 당시의 낮은 HP 비율을 사용하면 복원 후 HP가 낮게 표시되는 버그
        // → 장비 스탯 최종 반영 후 HP/마나 강제로 최대치로 재설정
        ...
        if (ps != null)
        {
            ...
            ps.UpdateStatsUI();
            Debug.Log($"[SaveLoadManager] HP/마나 최대치 복원: {ps.maxHealth}/{ps.maxMana}");
        }
    }
